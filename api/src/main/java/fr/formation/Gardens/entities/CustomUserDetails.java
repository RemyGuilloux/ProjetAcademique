...
public class CustomUserDetails extends User {
	...
	private static Set<GrantedAuthority> buildAuthorities(Set<Role> roles) {
		return roles.stream().map(r -> new SimpleGrantedAuthority(r.getCode())).collect(Collectors.toUnmodifiableSet());
	}
	...
	public String toString() {
		return MessageFormat.format(
				"[ id: {0}, authorities: {1}, password: [PROTECTED], username: {2}, enabled: {3}, expired: {4}, locked: {5}, credentials-expired: {6} ]",
				getId(), getAuthorities(), getUsername(), isEnabled(), !isAccountNonExpired(), !isAccountNonLocked(),
